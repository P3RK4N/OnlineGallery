...
public class WebServer
{
    ...
    public async void Start()
    {
        ...
        /// </summary>
        application.MapGet("/", async (HttpContext context, UserManager userManager) =>
        {
        ...
        application.MapGet("/api/users/{username}/photos", async (string username, HttpContext context, UserManager userManager, DbContext dbContext) =>
        {
            ...
            var photos = dbContext.Photos.Where(p => p.UserId == users[0].Id).ToList().ConvertAll(p => new PhotoDTO
            {
                Id = p.Id,
                UserId = p.UserId,
                Content = p.Content,
        ...
        {
            var photos = dbContext.Photos.ToList().ConvertAll(p => new PhotoDTO
            {
                Id = p.Id,
                UserId = p.UserId,
                Content = p.Content,
        ...
        {
            var photo = await dbContext.Photos.Where(p => p.Id == photoID).ToListAsync();
        ...
        {
            var photoUserId = await dbContext.Photos.Where(p => p.Id == photoID).Select(p => p.UserId).ToListAsync();
        ...
        application.MapPost("/api/user/delete", async (HttpContext context, UserManager userManager, DbContext dbContext) =>
        {
            ...
            await dbContext.Entry(user).Collection(u => u.Photos).LoadAsync();
        ...
        application.MapPost("/api/photos/{photoID}/update", async (int photoID, HttpContext context, UserManager userManager, DbContext dbContext) =>
        {
            ...
            var pic = await dbContext.Photos.Where(p => p.Id == photoID).FirstOrDefaultAsync();
            if(pic == null)
            {
                context.Response.StatusCode = StatusCodes.Status400BadRequest;
        ...
        application.MapPost("/api/photos/{photoID}/delete", async (int photoID, HttpContext context, UserManager userManager, DbContext dbContext) =>
        {
            ...
            var photos = await dbContext.Photos.Where(p => p.Id == photoID).ToListAsync();
